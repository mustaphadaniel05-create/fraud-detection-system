...
Accepts faces with similarity ≥ 0.54.
"""
...
SAME_FACE_THRESHOLD = 0.90   # as requested
...
def verify_identity(email: str, frame: np.ndarray) -> Dict[str, Any]:
    ...
    try:
        with get_connection() as conn:
            with conn.cursor() as cursor:
                cursor.execute("SELECT id, face_embedding FROM users WHERE email = %s", (email,))
    ...
    logger.info(f"ArcFace | email={email} | raw={raw_similarity:.4f} | scaled={scaled_similarity:.4f} | threshold={SAME_FACE_THRESHOLD}")
